...
The service has both public/customer-facing reads and admin-facing writes. Admin authorization is intended but not fully implemented yet.
...
- `ProductController`: customer product search and lookup endpoints.
- `AdminProductController`: admin product mutation endpoints.
...
| `GET` | `/api/v1/products/{id}` | Get a product by UUID. |
| `GET` | `/api/v1/products/sku/{sku}` | Get a product by SKU. |
| `GET` | `/api/v1/categories` | List categories. |
| `GET` | `/api/v1/categories/{name}` | Intended category lookup by name. Current method reads `@RequestParam String name`, so this needs a path-variable fix. |
| `POST` | `/api/v1/admin/products` | Create a product. Should require admin authority. |
| `PUT` | `/api/v1/admin/products/{id}` | Update a product. Should require admin authority. |
| `DELETE` | `/api/v1/admin/products/{id}` | Soft-deactivate a product. Should require admin authority. |
| `PATCH` | `/api/v1/admin/products/{id}/inventory` | Temporarily set product inventory quantity until `inventory-service` exists. |

`ProductSearchRequest` query parameters:
- `category`
- `minPrice`
- `maxPrice`
- `search`
...
- Fix category lookup to use `@PathVariable`.
